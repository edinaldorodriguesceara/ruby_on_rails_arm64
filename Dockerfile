...
FROM --platform=linux/arm64 ruby:${ARG_RUBY_VERSION}-slim-buster
...
RUN wget -nv "https://nodejs.org/dist/v${ARG_NODE_VERSION}/node-v${ARG_NODE_VERSION}-linux-arm64.tar.gz" && \
  tar -xf "node-v${ARG_NODE_VERSION}-linux-arm64.tar.gz" --directory '/usr/local' --strip-components '1' && \
  rm -rf "node-v${ARG_NODE_VERSION}-linux-arm64.tar.gz"
